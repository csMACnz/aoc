...
fn parse_hand(line: &str) -> Hand {
    ...
    Hand {
        cards: hand.chars().collect::<Vec<char>>().try_into().unwrap(),
...
fn score(hand: &Hand) -> u64 {
    ...
    if groups.len() > 1 {
        if let Some(&j_count) = groups.get(&'J') {
            groups.remove(&'J');
            let max = groups.values().max().unwrap();
            for key in groups.keys() {
                if groups[key] == *max {
                    groups.insert(*key, groups[key] + j_count);
                    break;
                }
            }
        }
...
fn main() {
    let mut hands = parse_file("./src/bin/day7_part2/puzzle.txt");
